...
class AgingReportController extends Controller
{
    public function index()
    {
     ...
     public function entriesBetweenDates(Request $request){
        ...
        $vouchers = VoucherDetail::where('sub_account_id',$request->sub_account)->whereDate('date', '<=',date("Y-m-d"))->orderBy('date','desc')->get();
        $totalDebit = VoucherDetail::where('sub_account_id',$request->sub_account)->whereDate('date', '<=',date("Y-m-d"))->sum('debit_amount');
        $totalCredit = VoucherDetail::where('sub_account_id',$request->sub_account)->whereDate('date', '<=',date("Y-m-d"))->sum('credit_amount');
